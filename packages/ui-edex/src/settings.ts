...
/** The default theme color — the sonar station's pale cyan phosphor. */
export const DEFAULT_THEME_COLOR = '#8ED5DF'
...
export const THEME_COLOR_PRESETS: readonly ThemeColorPreset[] = Object.freeze([
  { id: 'analyzed', labelKey: 'edex.preset.analyzed', color: '#8ED5DF' },
...
/**
 * The semantic accents that stay fixed across theme colors — the sonar
 * station's own signal language (DEMON amber, ping red, waterfall blue).
 */
export const FIXED_ACCENTS = Object.freeze({
  amber: '#FACD78',
  red: '#E54344',
  cyan: '#74CEFF',
  /** The BTR recorder's phosphor green (bearing-time event lines). */
  green: '#03E877',
  /** The bright turquoise of active traces and selected modes. */
  active: '#00E6D0',
})
 ...
 *
 * The background tokens resolve to the WIDGET CARD surface (the analysis's
 * `panelTone`), not black — the sonar station's panels are slightly lighter
 * than the near-black canvas, and the workspace must read as the same surface
 * as the shell's cards, never a different black.
 */
export function tokenOverridesFor(palette: EdexPalette): ThemeTokenOverrides {
  return {
    // The workspace shares the shell's card surface (#141F26 in the static
    // skin; palette-driven when a theme color is set).
    ...
    '--dsw-alias-state-success-primary': both(FIXED_ACCENTS.green),
