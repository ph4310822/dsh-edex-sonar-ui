...
import type { CommandResult } from '@danielng23/dsh-sonar-host-system-metrics/types'
...
/** The workspace inset: 1px center frame + 19px title strip — the reshaped
 *  original UI starts BELOW the strip so the chrome frames without covering. */
...
export function EdexShell({
  ...
  useLayoutEffect(() => {
    ...
    // original UI starts below the center title strip (the chrome inset).
    frame.style.top = `calc(${TOP_BAR_HEIGHT} + ${CENTER_TITLE_INSET})`
  ...
  return (
    ...
    >
      ...
      {/* The ORIGINAL UI wrapped in the standard widget chrome: the center
          CONTAINER carries the card frame + the SONAR WORKSPACE title strip
          (the workspace itself is inset below the strip), and the inner
          WidgetSection stays fully transparent so the working area is never
          occluded. */}
      <section className={css.centerWidget}>
        <div className={css.centerTitle}>SONAR WORKSPACE</div>
        <WidgetSection slot={CENTER_SLOT}>
          {/* Same faint sonar-scanline texture as the panel cells, over the
