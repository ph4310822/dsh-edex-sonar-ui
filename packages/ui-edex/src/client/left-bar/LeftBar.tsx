/**
 * Left bar: a vertical stack of swappable sonar-station widgets — ship
 * intelligence (clock/uptime), the main drive control (RPM gauges), and the
 * contact registry table. The composition lives in the LEFT_WIDGETS registry
 * below: add, remove, or reorder a widget by editing one line (its
 * implementation lives in its own folder under widgets/), and every section
 * shares the same chrome via WidgetSection.
 */
...
const LEFT_WIDGETS: LeftWidgetSlot[] = [
  { id: 'info', title: 'TARGET ACOUSTIC INTEL', Component: InfoWidget },
  { id: 'cpu', title: 'MAIN DRIVE CONTROL', Component: CpuWidget },
  // Flex-fills the leftover bar height so the table runs into the loadavg
  // footer the widget itself renders.
  { id: 'processes', title: 'CONTACT REGISTRY', fill: true, Component: ProcessWidget },
]
